#include "depth_buffering.h"

namespace demo {
	void DepthBuffering::UpdateAndRender(spn::Canvas* canvas) {
		...
		rasterizer.SetTexture(&lenaTexture);
		...
		n3d::mat4x4 cmbtr;
		n3d::mat4x4 cmbtrs;
		float phasedangle = angle + 90;
		n3d::mat4x4_set_roation_x(rot, phasedangle);
		n3d::mat4x4_set_translation(trans, 1, 0.2, 0.2);
		n3d::mat4x4_set_scale(scale, 0.25, 0.25, 0.25);
		n3d::mat4x4_multiply(cmbtr, trans, rot);
		n3d::mat4x4_multiply(cmbtrs, cmbtr, scale);
		rasterizer.SetModelMatrix(cmbtrs);
		rasterizer.SetTexture(&checkerTexture);
		rasterizer.RenderMesh(debugMesh);

		if (renderDepth) {
			rasterizer.RenderDepth();
		}
		char buffer[256];
	...
	void DepthBuffering::HandleInput(const SDL_Event* sdlEvent) {
	...
	void DepthBuffering::Init(spn::SpinachCore* sc){
		spinachCore = sc;
		...
		fTexture.CreateFromPng("../res/f.png");
		lenaTexture.CreateFromPng("../res/lena_color.png");
		checkerTexture.CreateCheckerImage(128, 128, 16, 0, 0, 0, 255, 255, 255);
		rasterizer.SetTexture(&lenaTexture);
		//rasterizer.SetTexture(&fTexture);
		//rasterizer.SetTexture(&checkerTexture);
