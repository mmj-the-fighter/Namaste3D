...
namespace n3d {
	...
	public:
		...
		inline void MoveCamera(CamMoveDirection direction) {
			float dx = 0.0f;
			float dy = 0.0f;
			float dz = 0.0f;
			switch (direction) {
		...
		inline bool IsViewModified() {
			return isViewMatDirty;
		}
		...
		inline void SetCameraViewNeedsUpdate(bool flag) {
			isViewMatDirty = flag;
		}

		inline void SetCameraProjectionNeedsUpdate(bool flag) {
			isProjMatDirty = flag;
		}

	private:
		inline void UpdateAtBasedOnYawAndPitch() {
			vec3f cameraForward;
