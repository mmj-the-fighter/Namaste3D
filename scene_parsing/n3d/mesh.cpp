...
namespace n3d
{
	...
	void Mesh::LoadObjectFile(const char* filepath, n3d::vec3f& preScale){
		std::cout << "[]loading object file " << filepath << "\n";
		...
		int n = points.size();
		n3d::vec3f minp = points[0];
		n3d::vec3f maxp = points[0];
		for (int i = 1; i < n; i++) {
			const n3d::vec3f v = points[i];
		...
		float diagonal = vec3f_distance(&minp, &maxp);
		vec3f_lerp(&center, &minp, &maxp, 0.5);
		radius = vec3f_distance(&minp, &center);
	}
