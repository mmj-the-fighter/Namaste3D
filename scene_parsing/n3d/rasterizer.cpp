...
namespace n3d
{
	...
	bool Rasterizer::RenderMesh(Mesh& mesh) {
		bool isViewModified = camera->IsViewModified();
		camera->Sync(matView, matProjection);
		mat4x4_multiply(matViewModel, matView, matModel);
		
		//vfc extract viewspace frustum planes
		plane frustumPlanes[6];
		mat4x4_extract_clipping_planes(frustumPlanes, matProjection);

		//preparation for vfc: transform object space mesh center to view space 
		vec3f meshCenter = mesh.center;
		vec4f centerv4;
		centerv4.v[0] = meshCenter.v[0];
		centerv4.v[1] = meshCenter.v[1];
		centerv4.v[2] = meshCenter.v[2];
		centerv4.v[3] = 1;
		mat4x4_transform(&centerv4, matViewModel);
		meshCenter.v[0] = centerv4.v[0];
		meshCenter.v[1] = centerv4.v[1];
		meshCenter.v[2] = centerv4.v[2];

		//preparation for vfc: calculate scaled mesh radius
		vec4f right = { 1, 0, 0, 0 };
		vec4f up = { 0, 1, 0, 0 };
		vec4f back = { 0, 0, 1, 0 };
		mat4x4_transform(&right, matViewModel);
		mat4x4_transform(&up, matViewModel);
		mat4x4_transform(&back, matViewModel);
		float sx = vec3f_mag_squared((vec3f*)&right);
		float sy = vec3f_mag_squared((vec3f*)&up);
		float sz = vec3f_mag_squared((vec3f*)&back);
		float radiusScale = sqrt(MaxOfThree(sx, sy, sz));
		float meshRadius = mesh.radius * radiusScale;

		//vfc
		for (int i = 0; i < 6; i++) {
			float d = plane_signed_distance(&frustumPlanes[i], &meshCenter);
			if ( d < -EPSILON) {
				if ( d < -meshRadius) {
					return false;
				}
		...
		//preparation for vertex lighting: calculate normal matrix
		mat3x3 upperLeftViewModelMat;
		mat4x4_extract_upper_left_3x3(upperLeftViewModelMat, matViewModel);
		mat3x3 inv;
		mat3x3_inverse(inv, upperLeftViewModelMat);
